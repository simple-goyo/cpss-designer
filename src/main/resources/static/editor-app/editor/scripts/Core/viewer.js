 ...
 */
if(!ORYX) {var ORYX = {};}
var Viewer_loadContentFinished = false;
...
ORYX.Viewer = {
    ...
    construct: function() {
        ...
        div.addClassName("ORYX_Viewer");
        let canvasStencil = ORYX.Core.StencilSet.stencil("http://b3mn.org/stencilset/bpmn2.0#BPMNDiagram");
        ...
        window.setTimeout(function(){
            ...
            Viewer_loadContentFinished = true;
            initFinished();
        }.bind(this), 200);
    },

    _myCreateCanvas: function (options ,stencil, facade){
        ...
        this.rootNode = ORYX.Editor.graft("http://www.w3.org/2000/svg", options.parentNode,
            ['svg', {id: this.id, width: options.width, height: options.height, class:"parentNodeClass"},
                ['defs', {}]
            ]);
        ...
        this._htmlContainer = ORYX.Editor.graft("http://www.w3.org/1999/xhtml", options.parentNode,
            ['div', {id: "viewer_htmlContainer", style: "position:absolute; top:5px"}]);
    ...
    _generateGUI: function() {
        ...
        jQuery("#Viewer").append(canvasParent);
...
ORYX.Viewer = Clazz.extend(ORYX.Viewer);

ORYX.MyCanvas = ORYX.Core.AbstractShape.extend({
    zoomLevel: 1,
